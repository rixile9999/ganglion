...
도구 정의는 `rlm_poc/dsl/tool_spec.py`의 `ToolSpec` 레지스트리를 통해 선언적으로 관리하며, JSON DSL catalog 텍스트와 OpenAI tool schema는 `Catalog.render_json_dsl()` / `Catalog.render_openai_tools()`로 동일한 source-of-truth에서 생성된다. 따라서 두 표현 간 비교는 apples-to-apples다.

### 14.2 카탈로그 사이즈 측정 (offline, deterministic)

`python -m rlm_poc.eval.scaling` 결과:
...
```bash
python -m rlm_poc.eval.runner --llm rules --tier iot_light_5
python -m rlm_poc.eval.runner --llm rules --tier home_iot_20
python -m rlm_poc.eval.runner --llm rules --tier smart_home_50
```
...
`rlm_poc.eval.runner`에 `--repeat N` flag를 추가했다. 각 case를 N회 호출한 뒤 mean / p50 / p95 / stddev를 모두 합산해 단일 summary로 출력한다.
...
```bash
python -m rlm_poc.eval.runner --llm rules --tier iot_light_5 --limit 50 --repeat 5
```
...
`rlm_poc/runtime/qwen.py`에 `run_dsl_with_repair(catalog, user_prompt, completer, repair_config)` 헬퍼를 추가했다. validator가 `DSLValidationError`를 던지면 다음 메시지를 추가해 동일한 `Completer`를 재호출한다.
...
```bash
python -m rlm_poc.eval.runner --llm qwen --tier iot_light_5 --repair --repair-max-attempts 1
```
...
```text
rlm_poc/dsl/tool_spec.py     (신규) ToolSpec, EnumArg, IntArg, StringArg, TimeArg, RawArg
rlm_poc/dsl/catalog.py       (신규) Catalog: render_json_dsl, render_openai_tools, validate
rlm_poc/dsl/validator.py     (리팩터) iot_light catalog로 위임하는 thin shim
rlm_poc/schema/iot_light.py  (리팩터) ToolSpec list로 재정의, JSON_DSL_CATALOG/OPENAI_TOOLS는 derive
rlm_poc/schema/home_iot.py   (신규) tier 20 catalog
rlm_poc/schema/smart_home.py (신규) tier 50 catalog
rlm_poc/schema/__init__.py   (확장) get_catalog(tier), TIERS registry
rlm_poc/runtime/qwen.py      (확장) Catalog 인자, run_dsl_with_repair, RepairConfig
rlm_poc/eval/metrics.py      (확장) RunResult, repeat-aware CaseResult, mean/stddev/repair stats
rlm_poc/eval/runner.py       (확장) --tier, --repeat, --repair, --repair-max-attempts
rlm_poc/eval/scaling.py      (신규) catalog 사이즈 측정 CLI
