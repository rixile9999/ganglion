...
This module also exposes the runtime wiring that hands an XGrammar grammar to
the HuggingFace ``model.generate()`` API. Two-step usage to amortize the
expensive compile cost across many ``model.generate()`` calls:
